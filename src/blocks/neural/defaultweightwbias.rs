...
{
    let sum = DefaultWeightwBiasFunction::calc_weight(I8_VECTOR1 , I8_VECTOR3 ) ;
}

#[test]
fn test_default_weight_function_i8() {
    //let weightFunction : &WeightFunction<f32 ,f32 > = &DefaultWeightwBiasFunction;

    info!("running default_weight_tests");


    let sum = DefaultWeightwBiasFunction::calc_weight(I8_VECTOR1 , I8_VECTOR1 ) ;
    assert_eq!(1, sum);
        let sum = DefaultWeightwBiasFunction::calc_weight(&[1 ,2, 3 ] , &[1,2,3] ) ;
        assert_eq!(14, sum);
        let sum = DefaultWeightwBiasFunction::calc_weight(&[1f32 ; 3 ] , &[1f32 ; 3 ] ) ;
        assert_eq!(3f32, sum);
}
...
//uper::block::
// #[derive(Copy, Cltion = DefaultWeightwBiasFunction;
