...
impl<S: ISolver> Solver<S>{
    ...
    fn init_train_net(config: &mut SolverConfig, net: &mut Module) {
        // Caffe
    ...
    // solver
    pub fn solve(&mut self,  examples: &[(Vec<f32>, Vec<f32>)] ) {
        info!("Solving {}", self.net.name);

        let num_iter = 100;
        self.step(num_iter , examples);
    }

    fn step(&mut self, iters: usize ,  examples: &[(Vec<f32>, Vec<f32>)]) {
...
fn calc_loss(block : &ErrorInfo) -> f32
{
    0f32
}
